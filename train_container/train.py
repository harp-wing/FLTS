...
def train(model: torch.nn.Module,
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % log_per == 0:
            ...
            log(INFO, f"Epoch {epoch + 1} [Test]: loss {test_loss:.4f}, mse: {test_mse:.4f}, rmse: {test_rmse:.4f}")
            
            mlflow.log_metrics({"train_loss": train_loss_eval, "test_loss": test_loss, "train_mse": train_mse, "test_mse": test_mse}, step=epoch + 1)
